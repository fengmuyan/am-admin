  ...
  <div class="order-detail-wrap app-container wipe">
    <div class="order-detail" v-loading="loading" v-if="goodsList">
      <div class="statusStep">
        <div class="statusLeft">
          ...
          <p v-if="Number(tradestate) === 2" class="confirm-tip">
            5天用户未确认收货由系统自动完成确认!
          </p>
        </div>
            ...
            <template slot-scope="scope">
              ...
              <div class="content">
                ...
                <p>
                  ...
                  <br />
                  <span class="refund-pro" v-if="Number(scope.row.refund) !== 0">{{
                    scope.row.refund | initRefund
                  }}</span>
                </p>
              </div>
            </template>
          ...
          <el-table-column label="操作" width="220px" v-if="wipeAction || isShowAction">
            <template slot-scope="scope">
              ...
              <el-button
                type="text"
                v-if="
                  Number(scope.row.tradestate) === 0 ||
                    Number(scope.row.tradestate) === 10 ||
                    Number(scope.row.tradestate) === 11
                "
      ...
      <div class="footer" v-if="delivertype !== 0 || invocetype !== 0">
        <div class="footer-left" v-if="delivertype !== 0">
          ...
          <p>
            <b>收货地址：</b>
            <span
              >{{ deliveryAddress.province }}{{ deliveryAddress.city }}{{ deliveryAddress.area
              }}{{ deliveryAddress.address }}</span
            >
          </p>
        </div>
      ...
      <el-form :model="weightForm" ref="weightForm" :rules="weightFormRules" label-width="140px">
        <div class="goods-info">
          <span>
            ...
            <b>{{ weightForm.grossweight }}</b> </span
          >&nbsp;&nbsp;&nbsp;&nbsp;
          <span>
            商品净重（{{ weightForm.weightunit }}）：
            <b>{{ weightForm.netweight }}</b> </span
          >&nbsp;&nbsp;&nbsp;&nbsp;
          <span>
            商品单价：
            <b>￥{{ weightForm.cmdtprice }}</b> </span
          >&nbsp;&nbsp;&nbsp;&nbsp;
          <span>
            折扣：
            <b>{{ weightForm.discount | initDiscount }}</b> </span
          >&nbsp;&nbsp;&nbsp;&nbsp;
          <span>
            数量：
            <b>{{ weightForm.cmdtcount }}</b> </span
          >&nbsp;&nbsp;&nbsp;&nbsp;
          <span>
            商品总价：
            <b>￥{{ weightForm.cmdttotalprice }}</b> </span
          >&nbsp;&nbsp;&nbsp;&nbsp;
          <span>
            优惠价格：
            <b>￥{{ weightForm.couponprice }}</b> </span
          >&nbsp;&nbsp;&nbsp;&nbsp;
          <el-button
            icon="el-icon-plus"
            size="mini"
        ...
        <el-form-item label="订单确认总价：" prop="adjustedprice">
          ...
          <div class="tip">* 确认价格可抹零，与称重后用户价格上下相差不能超过500元</div>
        </el-form-item>
      </el-form>
    ...
    <el-dialog title="抹账" :visible.sync="openWipe" @close="clearValidateWipe" width="550px">
      <el-form :model="wipeForm" ref="wipeForm" :rules="wipeFormRules" label-width="120px">
        ...
        <div style="line-height: 35px;color: rgb(200,200,200);padding-left: 120px;">
          将为你手机号{{ numberInit }}发送验证码，请注意查收。
        </div>
        <el-form-item label="短信验证" prop="smsCode" class="code-item">
          <el-input
            v-model="wipeForm.smsCode"
            maxlength="6"
            placeholder="输入验证码"
            class="code-input"
          ></el-input>
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" :loading="loadingWipe" @click="submitWipeForm('wipeForm')"
          >确 定</el-button
        >
      </div>
    </el-dialog>
...
export default {
  ...
  data() {
    ...
    const validateAdjust = (rule, value, callback) => {
      ...
      if (!patterInt.test(value)) {
        ...
        if (Math.abs(value - this.weightForm.weighedCtp) > 500) {
          callback(new Error('上下浮动不能大于500！'))
        }
        callback()
      }
    }
    const validateAmount = (rule, value, callback) => {
      if (!patterAmount.test(value)) {
        ...
        callback(new Error(`抹账金额不能大于最大抹账金额（${this.wipeForm.pendingAmount}）！`))
      } else {
        callback()
      }
    }
    return {
      ...
      wipeFormRules: {
        wipeAccounts: [{ validator: validateAmount, required: true, trigger: 'blur' }],
  ...
  filters: {
    ...
    initRefund(val) {
      const arr = ['正常', '退款审核中', '退款审核失败', '退款中', '已退款', '退款失败']
      return arr[val]
    }
  },
  computed: {
    ...
    ac12411() {
      ...
      return tradestate === 11 || tradestate === 1 || tradestate === 2 || tradestate === 4
    },
    ac124() {
      const tradestate = this.tradestate
    ...
    ac1234() {
      const tradestate = this.tradestate
      return tradestate === 1 || tradestate === 2 || tradestate === 3 || tradestate === 4
    },
    wipeAction() {
    ...
    async getDetail() {
      try {
        ...
        if (code === 200) {
          ...
          this.totalAdjustPrice = cmdtOrderDetailRespList.reduce((pre, item) => {
            pre = accAdd(pre, Number(item.couponprice))
            return pre
          }, 0)

          this.totalWeightPrice = cmdtOrderDetailRespList.reduce((pre, item) => {
          ...
          cmdtOrderDetailRespList.forEach(item => {
            Object.assign(item, {
              standards: JSON.parse(item.saleprovalue).salepro.reduce((pre, v) => {
    ...
    handelWeighedGwChange(val) {
      ...
      this.timer = setTimeout(() => {
        ...
        if (val && patter.test(val) && val > frameWeight) {
          ...
          const weighedCtp = subtr(accMull(accMull(weighedCp, discount), cmdtcount), couponprice)
          Object.assign(this.weightForm, {
            weighedNw,
            weighedCp,
            weighedCtp,
      ...
      this.$refs[formName].validate(async valid => {
        if (valid) {
          try {
            ...
            const { orderno, detailno, wipeAccounts, smsCode, remarks } = this.wipeForm
